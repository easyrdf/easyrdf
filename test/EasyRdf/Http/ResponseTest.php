...
class EasyRdf_Http_ResponseTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$response = EasyRdf_Http_Response::fromString(readFixture('http_response_200'));
		$this->assertEquals('OK', $response->getMessage(), 'Message is expected to be OK');
	}
	...
	{
		$response = EasyRdf_Http_Response::fromString(readFixture('http_response_404'));
	...
	{
		$response = EasyRdf_Http_Response::fromString(readFixture('http_response_500'));
	...
	{
		$response = EasyRdf_Http_Response::fromString(readFixture('http_response_302'));

		$this->assertEquals(302, $response->getStatus(), 'Response code is expected to be 302, but it\'s not.');
		$this->assertEquals('http://localhost/new/location', $response->getHeader('Location'), 'Response code is expected to be 302, but it\'s not.');
	...
	public function testGetHeaders()
	{
		...
		$this->assertEquals(7, count($response->getHeaders()), 'Header count is not as expected');
